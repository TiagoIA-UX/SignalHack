...
export async function POST(req: Request) {
  ...
  try {
    ...
    captureException(err, { requestId, path: "/api/auth/request", method: "POST", status: 500, ip, ua });
    logEvent("error", "auth.magic.request.unhandled", { requestId, path: "/api/auth/request", method: "POST", status: 500, ip, ua, extra: { err: errMsg } });
    await logAccess({ path: "/api/auth/request", method: "POST", status: 500, ip, ua });
